...
describe("every F write helper encodes against the generated ABI", () => {
  ...
  it("the lifecycle: initialize / seed / dissolve / proposeAuthority / acceptAuthority", async () => {
    ...
    await expect(
      fPoolInitialize(c, {
        ...
        phi: 10n ** 16n, kind: "pad", authority: A(0xa), account: A(0xa),
      }),
    ).resolves.toBe(HASH);
  ...
  it("the launch: one multicall of initialize / seed / the seed lock / the opening buy / the name / the gates", async () => {
    const { c, encoded } = fakeClient();
    const key = { quote: A(0xc), bases: [A(0xb), A(0x9)], weights: [4n * 10n ** 17n, 6n * 10n ** 17n], phi: 10n ** 16n, kind: "pad", authority: A(0xa) };
